...
export default function LoginPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (isLoginMode) {
      ...
      if (error) {
        // Cek apakah error-nya karena email sudah dipakai
        if (error.message.includes("already registered")) {
          alert(
            "Email ini sudah terdaftar! Silakan langsung Sign In atau gunakan tombol Google.",
          );
          setIsLoginMode(true); // Otomatis pindah ke tampilan Login
        } else {
          alert("Gagal register: " + error.message);
        }
      } else {
        alert(
          "Berhasil! Silakan cek inbox email kamu untuk verifikasi sebelum login.",
        );
        setIsLoginMode(true);
        setPassword("");
      }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6">
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm text-center">
        ...
        <p className="text-slate-500 mb-8 text-sm">
          {isLoginMode
            ? "Welcome back! Let's track."
            : "Join us to track your flow."}
        </p>
